...
Bevy的目的是针对不同的平台,让其操作简单,
比如不同的台式操作系统/web浏览器(通过WebAssembly)/移动端(Android和iOS)/以及游戏控制台.
...
Bevy在web平台使用WebAssembly运行得很好,但是也有一些限制.

不支持多线程,所以你只有限定的性能,和可能的audio glitches(
音频故障audio glitch通常指音频播放或录制过程中出现的短暂/不正常的声音干扰).

渲染受限于WebGL2 API的特性,这意味着更糟的性能和限制,比如在3D场景中,
...
很好地支持Apple iOS,很多特性运行得不错.
Bevy社区有些开发者已经成功地将基于Bevy的apps发布到App Store上.
...
不幸的是,由于NDA需求(保密协议,Non-Disclosure Agreement),
对于很多开放社区的开发者而言,基于控制台的研发是不可获得的.

Rust编程语言致力于支持Nintendo Switch,但是工作进展仍然停留在早期阶段,
并没有太多的进展,使其可用于Bevy.

Steam Deck和其他的一些手柄PCs,被很好地支持.
这些设备运行在特定版本的桌面OSs内.
